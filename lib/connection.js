...
function Connection (a, b) {
	...
	//if no b - treat connection as hangling connection
	if (!b) {
		self.state = 'hangling';
	} else if (a && b) {
		...
		before: function () {
			...
			on(document, 'mousemove.' + self.id, function (e) {
				...
				q.all('.block-in:not([hidden])', app.container).forEach(function (el) {
					...
					if (blockEl === self.element) return;
					...
					//for short distance - set angle to source node
					if (d < 80) {
				...
				if (closestBlock.inputs.size >= closestBlock.inputNumber) {
					return;
				}
