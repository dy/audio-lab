...
proto.connect = function (a, b) {
	...
	self.inEl.classList.add('connection-end');
	self.outEl.classList.add('connection-end');
 ...
 */
proto.update = function (fromCoord, toCoord, outEl, inEl) {
	var self = this;
	...
	css(outEl, 'transform', `rotate(${ angle }rad)`);
	css(inEl, 'transform', `rotate(${ angle + Math.PI }rad)`);
