...
class Connection extends Emitter {
	constructor (a, b) {
		...
		self.lab = self.from.lab;

		//generate unique connection id
		self.id = getUid();

		//save item by id
		self.lab.items[self.id] = self;
	...
	createElement () {
		...
		`);
		self.lab.element.appendChild(self.element);
	...
	updateEnd (e) {
		...
		q.all('[data-block-input]:not([hidden])', self.lab.element).forEach(function (el) {
		...
		if (self.lastDroppable && self.lastDroppable != closestBlockEl) {
			self.lastDroppable.classList.remove('block-connection-target');
	...
	destroy () {
		...
		self.lab.element.removeChild(self.element);
		delete self.lab.items[self.id];
	...
	toJSON () {
		...
		var data = {
			...
			className: Connection.displayName
		};
	...
	fromJSON (data) {
		...
		self.connect(self.lab.items[data.from], self.lab.items[data.to]);
		...
		before: function () {
			...
			on(document, 'mouseup.' + self.id, function (e) {
				...
				if (self.lastDroppable) {
					...
					//WARNING: emulate hover (fix browsers unstyled hover bug)
					...
					setTimeout(function () {
						...
						setTimeout(function () {
							off(self.to.element, 'mouseleave.' + self.id);
						}, 1000);
					})
