...
class Lab extends Emitter {
	constructor (options) {
		...
		var generator3 = new Generator(function (t) {
			// var sum = 0;
			// for (var i = 200; i < 600; i+=5) {
			// 	sum += Math.cos(Math.PI * 2 * t * i);
			// }
			// return sum / ((600 - 200) /5);
			return Math.sin(Math.PI * 2 * t * 432);
		})
		var mixer = new Mixer({
		});
		...
		//30 more listeners uses all the processor resource
		// for (var i = 400; i < 430; i++) {
		// 	var generator = new Generator(function (t) {
		// 		return Math.cos(Math.PI * 2 * t * i);
		// 	});
		// 	generator.pipe(mixer);
		// }
