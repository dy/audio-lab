...
class Lab extends Emitter {
	constructor (options) {
		...
		var Generator = require('../plugin/generator');
		var Output = require('../plugin/output');
		var Mixer = require('../plugin/mixer');
		var Gain = require('../plugin/gain');

		var generator1 = new Generator(function (t) {
			var f = Math.cos(Math.PI * 2 * t * 12) * 2;
			var f2 = Math.cos(Math.PI * 2 * t * 1) * 200;
			return isActive ? Math.cos(Math.PI * 2 * t * 800 + f + f2):0;
		});
		var generator2 = new Generator(function (t) {
			return isActive ? Math.random() * 2 - 1 : 0;
		});
		var generator3 = new Generator(function (t) {
			var f = Math.cos(Math.PI * 2 * t * 440);
			return isActive ? f : 0;
		})
		var mixer = new Mixer({
		});
		...
		generator1.pipe(mixer);
		generator2.pipe(mixer);
		generator3.pipe(mixer);

		mixer.pipe(gain).pipe(output);
	...
	use (block) {
		...
		//save block
		var blockName = self.getBlockName(block.displayName || block.name);
		...
		var itemEl = domify(`
			<div class="lab-appender-item"
				data-append-block
				title="${ capfirst(blockName.toLowerCase()) }">
			${ thumbnail }
			</div>
		`);
		...
		on(itemEl, 'click', function () {
			var block = self.createBlock(blockName);

			//set focus on block name
			block.label.click();
	...
	createDefaultProject () {
		...
		var generator = self.createBlock('Processor');
	...
	createBlock (name, options) {
		...
		//update autosize etc
		//FIXME: place autosize.update into block.update
		autosize(block.label);
		block.update();
