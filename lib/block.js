...
class Block extends Emitter {
	...
	createElement () {
		...
		self.element = domify(`
			<div class="block block-${ self.constructor.name.toLowerCase() }"
				...
				<textarea class="block-title" data-block-title rows="1">${ self.title || dragon.allRandom() }</textarea>
		...
		//get title element, autosize it
		self.title = q('[data-block-title]', self.element);
		on(self.title, 'change', function () {
			self.emit('change');
		});

		//select all by focus
		on(self.title, 'click', function () {
			setCaret(self.title, 0, 9999);
		});

		on(self.title, 'keydown', function (e) {
	...
	update () {
		...
		//FIXME: update name size
		// autosize.update(self.title);
	...
	createConnection (to) {
		...
		once(connection, 'connected', function () {
			self.emit('connect');
		});
	...
	start () {
		...
		if (self.node) {
			...
			self.outputs.forEach(function (connection) {
				//connect each node
				self.node.connect(connection.to.node);

				connection.to.start();
			});

			//connect inputs
			self.inputs.forEach(function (connection) {
				connection.from.node.connect(self.node);
			});
	...
	stop () {
		...
		//disconnect each input
		self.inputs.forEach(function (connection) {
			connection.from.node.disconnect();
		});
