...
proto.createElement = function () {
	...
	for (var i = 0; i < self.outputNumber; i++) {
		self.element.appendChild(domify(`<div class="block-out"></div>`));
	}
	for (var i = 0; i < self.inputNumber; i++) {
		self.element.appendChild(domify(`<div class="block-in"></div>`));
	}
	...
	delegate(self.element, 'mousedown', '.block-out', function (e) {
		...
		selection.disable(document.documentElement);
...
proto.connect = function (block) {
	...
	//create connection
	self.outputs.add((new Connection()).connect(self, block));
