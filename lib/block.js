...
class Block extends Emitter {
	constructor (options) {
		...
		//take over passed options
		extend(self, options);

		if (!self.lab) {
			throw Error('Audio-lab instance should be passed as `lab` option');
		}
 ...
 * Construct DOM representation
	...
	toJSON () {
		...
		var data = {
			...
			//className to create
			className: self.constructor.displayName
		};
