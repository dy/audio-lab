...
function Block (options) {
	...
	//bind global events
	on(app, 'start.' + self.id, function () {
		self.start();
	});

	on(app, 'stop.' + self.id, function () {
		self.stop();
	});
...
proto.createElement = function () {
	...
	self.draggable = new Draggable(self.element, {
		...
		threshold: 10,
		css3: false,
		handle: [`[data-from="${ self.id }"]`, `[data-to="${ self.id }"]`, q(`.block-thumbnail`, self.element)]
	});
	...
	delegate(self.element, 'mousedown.' + self.id, '.block-out', function (e) {
		new Connection(self);
	});
...
proto.destroy = function () {
	...
	off(self.element, 'mousedown');
	off(self.element, 'keydown');
	off(self.element, 'dblclick');
	off(self.draggable, 'drag.' + self.id);
	off(app, 'start.' + self.id);
	off(app, 'stop.' + self.id);
